...
def func_global():
    ...
    # 파이썬에서는 변수 = 값 선언 구분은 새로운 변수 생성(할당)임
    gnum = 200 # 지역변수
    print(f'gnum : {gnum}') # UnboundLocalError: cannot access local variable 'gnum' where it is not associated with a value
...
# 함수쪼겡서 함수 호출부(실행위치)의 변수 값 변경 못 함
