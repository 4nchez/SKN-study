...
print(ss)
print(ss[::-1]) # 문자열 전체임, 간격이 음수이면 뒤에서부터 추출
...
print(tt3.title()) # 각 단어의 첫글자를 대문자로, 나머지는 소문자로 변환
...
# 여러 개의 분리된 문자값들을 리스트(list, []로 표현)

# splitlines() : 줄(line) 단위로 분리해서 (한 줄씩 분리함) 리스트로 변환함
tt6 = '''python
java
c++
javascript
