...
print(s)
s[3] = np.nan   # index 3 에 NaN 기록함
print(s)
print(s.count())  # 9

# DataFrame 에 count 는 각 열별 데이터 갯수임
# 데이터프레임에 값이 누락된 부분을 찾을 때 유용함
np.random.seed(0)
...
# s2 시리즈의 값에 따른 데이터 갯수를 인덱스로 정렬한다면
print(s2.value_counts())
print(s2.value_counts().sort_index())  # 인덱스를 오름차순으로 정렬

# NaN 이 있는 경우에는 정렬하면 NaN이 가장 나중에 배치됨
print(s)
...
print(df2.sum(axis=1))  # 행별 합계 => 열이 축소됨 (8개 => 1개)
...
# apply() 함수
# 행이나 열단위로 좀 더 복잡한 계산을 적용해야 할 때 사용하는 함수
# 복잡한 계산식은 사용자정의함수로 작성하거나, 람다함수(이름없는 함수)를 사용함
df3 = pd.DataFrame({
    'A': [1, 3, 1, 3, 4],
    'B': [2, 3, 1, 2, 3],
    'C': [1, 3, 2, 4, 4]
