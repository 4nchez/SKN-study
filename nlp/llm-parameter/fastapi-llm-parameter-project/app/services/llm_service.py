...
def _openai_safe_usage(resp) -> dict:
    """OpenAI 응답에서 토큰 사용량 정보를 안전하게 추출합니다."""
    usage = getattr(resp, "usage", None)
    if usage is None:
        return {"prompt_token_count": None, "candidates_token_count": None, "total_token_count": None}
    return {
...
def openai_basic_call(prompt: str, temperature: float = 0.7, max_output_tokens: Optional[int] = 300) -> dict:
    ...
    try:
        ...
        # OpenAI Chat Completions API에 요청을 보냅니다.
        ...
        # 답변 텍스트와 토큰 정보를 합쳐 반환합니다.
        return {"text": resp.choices[0].message.content, **_openai_safe_usage(resp)}
    except Exception as e:
        traceback.print_exc()
        raise


def openai_role_chat(system_instruction: str, user_message: str) -> dict:
    ...
    # 환경변수의 OPENAI_API_KEY를 자동으로 읽어 클라이언트를 생성합니다.
        ...
        return {"text": text, **_openai_safe_usage(resp)}
    except Exception as e:
        traceback.print_exc()
        raise
