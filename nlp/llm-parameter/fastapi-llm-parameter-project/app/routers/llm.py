...
def call_openai_basic(request: BasicPromptRequest):
    """OpenAI 기본 호출을 테스트합니다."""

    try:
        return openai_basic_call(
            prompt=request.prompt,
            temperature=request.temperature,
            max_output_tokens=request.max_output_tokens,
        )
...
@router.post("/openai/role", response_model=LLMResponse)
def call_openai_role(request: RoleChatRequest):
    """OpenAI 키가 있을 때 OpenAI 호출 구조를 테스트합니다."""

    try:
        return openai_role_chat(
            system_instruction=request.system_instruction,
            user_message=request.user_message
        )
    except Exception as exc:
...
@router.post("/openai/diversity", response_model=DiversityResponse)
def check_openai_temperature_diversity(request: DiversityRequest):
    """temperature 값에 따른 답변 다양성을 측정합니다."""

    try:
        return openai_temperature_diversity(
            prompt=request.prompt,
            temperature=request.temperature,
            repeat_count=request.repeat_count,
        )
...
@router.post("/openai/token-compare")
def compare_openai_korean_english_tokens(request: TokenCompareRequest):
    """한국어와 영어 입력의 토큰 사용량을 비교합니다."""

    try:
        return openai_token_compare(
            korean_text=request.korean_text,
            english_text=request.english_text,
        )
    except Exception as exc:
