...
from app.data import load_news_data
...
def train_model(config: Config) -> Tuple[TextLSTMClassifier, Dict[str, object]]:
    """기사 데이터를 사용해 LSTM 문서 분류 모델을 학습한다."""

    set_seed(config.random_state)                                      # 실험 재현성을 위해 난수를 고정한다.
    raw_texts, labels = load_news_data()                             # 기사 문장과 카테고리 라벨을 불러온다.
    ...
    for epoch in range(1, config.epochs + 1):                         # 지정한 epoch 수만큼 전체 학습 데이터를 반복 학습한다.
        ...
        all_preds, all_targets, accuracy, val_loss = eval_model(config, model, test_loader, criterion)
        all_train_losses.append(t_loss)
        all_valid_losses.append(val_loss)

        print(f"Epoch {epoch:02d}/{config.epochs} - train-loss: {t_loss:.4f} - valid-loss: {val_loss:.4f}") # epoch 평균 손실을 출력한다.

        print(classification_report(all_targets, all_preds, target_names=[id_to_label[i] for i in range(len(id_to_label))],
                                    zero_division=0))  # 상세 평가표를 출력한다.
        ...
        with open(config.model_path.replace(".pt", "_meta.pkl"), "wb") as f:  # 전처리와 예측에 필요한 부가 정보를 저장할 파일을 연다.
            pickle.dump({"vocab": vocab, "label_to_id": label_to_id, "id_to_label": id_to_label, "config": config},
                        f)  # 사전과 라벨 정보를 저장한다.

    metadata = {"vocab": vocab, "label_to_id": label_to_id, "id_to_label": id_to_label,
                "accuracy": accuracy}  # 호출자에게 반환할 메타데이터를 구성한다.

    save_loss_curve(all_train_losses, all_valid_losses, save_path=config.loss_curve_img_path)

    return model, metadata

def eval_model(config: Config, model:TextLSTMClassifier,
               test_loader: DataLoader, criterion
    ...
    return all_preds, all_targets, accuracy, v_loss
...
def save_loss_curve(train_losses,
    ...
    """ Train/Validation Loss 그래프를 이미지 파일로 저장 """
