...
class Config:
    ...
    max_vocab: int = 1000          # 토큰화에 사용할 최대 단어 수이다.
    ...
    epochs: int = 9                # 전체 학습 데이터를 몇 번 반복해서 학습할지 정한다.
    learning_rate: float = 0.009   # Adam 최적화 알고리즘의 학습률이다.
    test_size: float = 0.20        # 전체 데이터 중 평가 데이터로 사용할 비율이다.
    random_state: int = 42         # 실험 결과를 재현하기 위한 난수 고정값이다.
    model_path: str = "./models/naver_news_lstm_model.pt"  # 학습된 모델을 저장할 경로이다.
    loss_curve_img_path: str = "./loss_curve.png"
