'''
BOW(Bag of Words)
: 단어의 순서는 고려하지 않고, 단어가 몇 번 등장했는지에 집중하는 텍스트 수치화 방법임

DTM(Document-Term Matrix)
: 여러 문서의 BOW를 하나의 행렬로 모은 것

TF-IDF(Term Frequency-Inverse Document Frequency)
:TF는 특정 문서 안에서 단어가 얼마나 자주 등장하는지를 나타냄
-IDF는 전체 문서에서 흔한 단어의 가중치는 낮추고, 흔하지 않은 단어의 가중치는 높임
'''
import sys
import subprocess
...
def tokenize_korean(document):                 # 한국어 문장을 토큰 리스트로 변환하는 함수
    ...
    clean_document = document.replace('.', '')          # 마침표 제거(불필요한 기호 제거: 정제)
    clean_document = clean_document.replace(',', '')

    if KONLPY_AVAILABLE:
        return okt.morphs(clean_document)               # 형태소 단위로 분리해서 리턴
    return clean_document.split()                       # 공백 기준으로 단어들을 나눔(토큰화)
...
def build_bag_of_words(document):              # 하나의 문서에서 BOW 사전과 빈도 벡터를 만드는 함수
    ...
    for word in tokenized_document:                     # 토큰화된 단어를 하나씩 처리
        if word not in word_to_index:                   # 현재 사전에 등록되지 않은 단어라면
            ...
            bow.append(1)                               # 새 단어 등록에 대한 빈도 1 추가(인덱스 순번을 맞춰 저장)
        else:                                           # 현재 단어가 사전에 등록되어 있다면
            index = word_to_index[word]                 # 기존 등록된 단어의 순번을 조회함
            bow[index] += 1                             # 해당 인덱스 위치의 빈도를 1 증가시킴
    return word_to_index, bow                           # 단어 인덱스 사전과 빈도 벡터 반환

# BOW 사용 테스트
doc1 = "파이썬을 이용한 텍스트 빈도수 카운트 실습을 진행합니다."
vocab, bow = build_bag_of_words(doc1)                   # 예제 문장을 BOW 사전과 빈도 벡터로 변환함
print("입력 문장: ", doc1)                                # 원본 출력
...
print_section("2. CountVectorizer 를 이용한 BOW 생성")
corpus = ["you know I want your love. I love you"]
vec = CountVectorizer()
bow_matrix = vec.fit_transform(corpus).toarray()
# 코퍼스를 학습한 후 단어 빈도 행렬을 배열로 변환
print("입력 코퍼스: ", corpus)
print("bag of words matrix: ")
print(bow_matrix)       # 문서 단어 빈도 행렬 출력
print("Vocabulary: ")
print(vec.vocabulary_)  # CountVectorizer가 만든 단어 인덱스 사전
...
# nltk가 제공하는 불용어 사전을 이용해 불용어를 제거할 수도 있음
# CountVectorizer에서 제공하는 영어 기본 불용어를 사용할 수도 있음


# 4. 워드 임베딩
print_section("4. DTM 과 TF-IDF 실습")

docs = [
    "배우고 싶은 자연어",                       # 첫 번째 문서
    "딥러닝 머신러닝 배우고 싶은 강화학습",        # 두 번째 문서
    "자연어 처리 좋아요",                       # 세 번째 문서
    "배우고 싶은 딥러닝",                       # 네 번째 문서
]

vocab = sorted(set(word for doc in docs for word in doc.split()))
N = len(vocab)  # 단어 개수 저장
print("문서 목록")
for idx, doc in enumerate(docs, start=1):
    print(f"문서{idx}: {doc}")                # 번호: 문서 출력
print("Vocabulary: ", vocab)
print("문서 개수 N :", N)                       # 전체 문서 개수
