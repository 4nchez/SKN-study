...
def generate_chat_reply(message: str, history: List[ChatMessage],
    ...
    messages = [
        {
            "role": "system",
            "content": (system_instruction or "").strip() or "너는 한국어로 친절하고 정확하게 답변하는 FastAPI 기반 ChatGPT 챗봇이다.",
        }
    ]
    ...
    params = {
        "model": selected_model,                # 사용할 OpenAI 모델명을 지정합니다.
        "messages": messages,                   # 사용자 대화
        "temperature": temperature,             # 출력의 창의성과 무작위성을 조절합니다.
        "top_p": top_p,                         # 누적 확률 기반 샘플링 범위를 조절합니다.
        "max_output_tokens": max_output_tokens, # 최대 출력 길이를 제한합니다.
    }
