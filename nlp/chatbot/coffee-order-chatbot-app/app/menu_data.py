# 커피 주문 챗봇에서 사용할 메뉴 시드(seed) 데이터를 정의합니다.
# 이 데이터는 앱 최초 실행 시 menus 테이블이 비어 있을 때 한 번만 DB로 옮겨집니다.
# (app/services/menu_service.py의 seed_menus 함수 참고)
# 이후 메뉴 관리는 이 파일이 아니라 DB(menus 테이블)에서 이루어집니다.

# 메뉴 목록을 리스트 안의 딕셔너리 구조로 저장합니다.
COFFEE_MENU = [
    {
        "id": 1,  # 메뉴를 구분하기 위한 고유 번호입니다. (DB에서는 자동 증가 id로 대체됩니다)
