...
from app.models.models import User  # users 테이블 ORM 모델입니다.
from app.schemas import TokenResponse, UserCreate, UserResponse  # 요청/응답 스키마입니다.
from app.security import create_access_token, get_current_user, hash_password, verify_password  # 보안 유틸 함수입니다.
...
def read_my_info(current_user: User = Depends(get_current_user)):
    # 로그인한 본인의 회원 정보를 확인하는 API입니다. (Authorization: Bearer <token> 필요)
    return current_user
