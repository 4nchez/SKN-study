"""10_build_index_genai.py 역할: 문제 1 — FAISS 인덱스 생성·저장"""  # 이 파일은 배치 인덱싱 단계입니다.

from langchain_community.vectorstores import FAISS  # FAISS 벡터스토어 클래스를 불러옵니다.

from common import EXERCISE_FAISS_DIR, get_embeddings, load_and_chunk  # 공통 경로, 임베딩 생성, 문서 청킹 함수를 불러옵니다.

BASE_FILES = ["멤버십정책.pdf"]  # 최초 FAISS 인덱스에 넣을 샘플 문서 목록입니다.
...
def build_index() -> None:  # FAISS 인덱스를 만드는 배치 함수입니다.
    ...
    vs.save_local(str(EXERCISE_FAISS_DIR))  # 생성된 FAISS 인덱스를 디스크에 저장합니다.
    print(f"FAISS 인덱스 저장 완료: {EXERCISE_FAISS_DIR}")  # 저장 위치를 사용자에게 안내합니다.
