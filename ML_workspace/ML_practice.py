...
def get_counts_and_ratios(array):
    values, counts = np.unique(array, return_counts=True)
    ratios = counts / len(array)
    return pd.Series(counts, index=values), pd.Series(ratios, index=values)

train_counts, train_ratios = get_counts_and_ratios(y_train)
test_counts, test_ratios = get_counts_and_ratios(y_test)

# 갯수 기준 비교
df_compare = pd.DataFrame({'Train Count': train_counts, 'Test Count': test_counts})
print(" 정답 데이터 카테고리 분포 비교 ")
print(df_compare)

print("\n" + "="*40 + "\n")

# 비율 기준 비교
df_ratio_compare = pd.DataFrame({'Train Ratio': train_ratios, 'Test Ratio': test_ratios})
print(" 정답 데이터 카테고리 비율 비교 ")
print(df_ratio_compare)
