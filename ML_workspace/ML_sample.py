...
# 머신러닝 모델, 데이터셋, 평가 도구를 제공함
from sklearn.datasets import load_iris
# 데이터를 학습용(훈련용)과 테스트용(평가용)으로 나누는 함수
from sklearn.model_selection import train_test_split
# 데이터 값을 표준화하는 전처리 도구
from sklearn.preprocessing import StandardScaler
# 분류 문제에 많이 사용하는 머신러닝 모델
...
# iris 데이터셋 불러오기
# iris 데이터는 붓꽃의 꽃받침, 꽃잎 길이 / 너비를 이용해 품종을 분류하는 예제 데이터
iris = load_iris()
...
# 숫자로 된 target을 실제 품종이름으로 변환한 컬럼도 추가
...
print('\n 품종별 기초 통계량 : ')
print(df.groupby(['target_name']).describe().T)
...
# 변수 간 관계를 산점도 행렬로 확인 : hue 는 품종별로 색상을 다르게 표시함
sns.pairplot(df, hue='target_name')
plt.show()

"""## 5. 입력 데이터와 정답 데이터 분리"""

# 머신러닝 모델에 넣을(읽힐 값) 값 (입력값) : target, target_name 은 정답이므로 제외함
x = df[feature_names]
...
x_train, x_test, y_train, y_test = train_test_split(
    ...
    stratify=y      # 품종 비율이 학습/테스트 데이터에 비숫하게 나뉘도록 설정
)
...
# StandardScaler 는 평균을 0, 표준편차를 1에 가깝게 변환함
...
# 테스트 데이터 기준으로 새로 fit하면 데이터 누수가 발생할 수 있음
...
# 4개의 특성이므로 4행 2열의 그래프 공간 생성
fig, axes = plt.subplots(nrows=4, ncols=2, figsize=(12, 16))

for i, col in enumerate(columns):
    # --- [왼쪽] 표준화 전 분포 ---
    sns.kdeplot(data=x_train, x=col, ax=axes[i, 0], color='skyblue', fill=True)
    axes[i, 0].set_title(f'Original: {col}')
    axes[i, 0].set_xlabel('') # 축 이름 생략으로 깔끔하게

    # --- [오른쪽] 표준화 후 분포 ---
    # x_train_scaled는 numpy 배열이므로 i번째 인덱스로 접근
    sns.kdeplot(x=x_train_scaled[:, i], ax=axes[i, 1], color='salmon', fill=True)
    axes[i, 1].set_title(f'Scaled: {col}')
    axes[i, 1].set_xlabel('')
    # 표준화 후에는 x축이 대략 -3 ~ 3 사이에 모이므로 축 범위를 통일해주면 보기 좋습니다.
...
# accuracy_score :
# 전체 테스트 데이터 중 몇 개를 맞혔는지 비율로 계산함
...
# f1-score : precision 과 recoll 을 종합한 점수

print(classification_report(y_test, y_pred, target_names=target_names))

"""## 13. Confusion Matrix(혼동 행렬) 확인"""
...
"""## 15. 모델 저장<br>
구글 코렙에서 파일 저장하면, 코렙의 가상 Linux 환경(임시 서버) 안에 저장됨
코렙 왼쪽의 파일 탐색기에서 확인할 수 있음
"""
